...
//#define QHLog(...)
#define kDBCloseTimeout         1000     // 100 times = 2ms

@implementation DSFMDatabase
...
- (id)initWithPath:(NSString*)aPath {
    ...
    if (self) {
        ...
        logsErrors          = 0x00;
        crashOnErrors       = 0x00;
        busyRetryTimeout    = kDBCloseTimeout;
    }
...
- (BOOL)open {
    ...
    QHLog(@"open db: %@, %d", databasePath, err);
	if(err != SQLITE_OK) {
		return NO;
	}
...
- (BOOL)openWithFlags:(int)flags {
    ...
    QHLog(@"open db 2: %@, %d, %d", databasePath, flags, err);
	if(err != SQLITE_OK) {
		return NO;
	}
	return YES;
...
- (BOOL)close {
    ...
    do {
        ...
        if (SQLITE_BUSY == rc) {
            retry = YES;
            usleep(20);
            if (kDBCloseTimeout && (numberOfRetries++ > kDBCloseTimeout)) {
                //QHLog(@"%s:%d", __FUNCTION__, __LINE__);
                QHLog(@"Database busy, unable to close");
                return NO;
            }
        }
        else if (SQLITE_OK != rc) {
            QHLog(@"error closing!: %d", rc);
        }
    }
    while (retry);
    
	db = nil;
    QHLog(@"close db end! %@", databasePath);
    return YES;
}
...
- (DSFMStatement*)cachedStatementForQuery:(NSString*)query {
    return [cachedStatements objectForKey:query];
}

- (void)setCachedStatement:(DSFMStatement*)statement forQuery:(NSString*)query {
    //QHLog(@"setting query: %@", query);
...
- (BOOL)rekey:(NSString*)key {
    ...
    if (rc != SQLITE_OK) {
        QHLog(@"error on rekey: %d", rc);
        QHLog(@"%@", [self lastErrorMessage]);
    }
...
- (BOOL)goodConnection {
    ...
    DSFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)compainAboutInUse {
    QHLog(@"The DSFMDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (crashOnErrors) {
        NSAssert1(false, @"The DSFMDatabase %@ is currently in use.", self);
    }
#endif
}
...
- (BOOL)executeSql:(NSString *)sql error:(NSError**)outErr withBindHandler:(BOOL (^)(sqlite3_stmt *stmt))handler
{
    ...
    DSFMStatement *cachedStmt = 0x00;
    
    if (traceExecution && sql) {
        QHLog(@"%@ executeUpdate: %@", self, sql);
    }
        ...
        do {
            ...
            if (SQLITE_BUSY == rc) {
                ...
                if (busyRetryTimeout && (numberOfRetries++ > busyRetryTimeout)) {
                    QHLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [self databasePath]);
                    //QHLog(@"Database busy");
                ...
                if (logsErrors) {
                    QHLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                    QHLog(@"DB Query: %@", sql);
#ifndef NS_BLOCK_ASSERTIONS
                    if (crashOnErrors) {
    ...
    if ((handler)&&(!handler(pStmt))) {
        QHLog(@"Error: the bind count is not correct for the # of variables (%@) (executeUpdate)", sql);
        sqlite3_finalize(pStmt);
        [self setInUse:NO];
        return NO;
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc) {
            ...
            QHLog(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(db));
            QHLog(@"DB Query: %@", sql);
        }
        else if (SQLITE_MISUSE == rc) {
            // uh oh.
            QHLog(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(db));
            QHLog(@"DB Query: %@", sql);
        }
        else {
            // wtf?
            QHLog(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(db));
            QHLog(@"DB Query: %@", sql);
        }
    ...
    if (shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[DSFMStatement alloc] init];
...
- (DSFMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    DSFMResultSet *rs = nil;
    
    int rc                  = 0x00;;
    sqlite3_stmt *pStmt     = 0x00;;
    DSFMStatement *statement  = 0x00;
    
    if (traceExecution && sql) {
        QHLog(@"%@ executeQuery: %@", self, sql);
    }
    ...
    if (idx != queryCount) {
        QHLog(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        [self setInUse:NO];
        return nil;
    }
    ...
    if (!statement) {
        statement = [[DSFMStatement alloc] init];
    ...
    rs = [DSFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (void)makeFunctionNamed:(NSString*)name maximumArguments:(int)count withBlock:(void (^)(sqlite3_context *context, int argc, sqlite3_value **argv))block {
    ...
    if (!openFunctions) {
        openFunctions = [NSMutableSet new];
    }
    
    id b = [[block copy] autorelease];
    
    [openFunctions addObject:b];
